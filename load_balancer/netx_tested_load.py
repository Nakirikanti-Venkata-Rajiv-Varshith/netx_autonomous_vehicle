...
class LoadBalancerNode:
    def __init__(self, name):
        ...
        self._last_object_msg_time = time.time()
        threading.Thread(target=self._yolo_watchdog, daemon=True).start()
    ...
    def normalize_detections(self, detections, source="cloud"):
        ...
        return normalized
    def _yolo_watchdog(self):
        if rospy.get_param("~only_line_follow", False):
            return  # YOLO not used in this mode
        ...
        while self.is_running:
            time.sleep(3.0)
            age = time.time() - self._last_object_msg_time
            if age > STALL_THRESHOLD and (time.time() - last_restart) > COOLDOWN:
                rospy.logwarn(
                    f"[watchdog] YOLO object stream stalled ({age:.1f}s), "
                    "attempting restart..."
                )
                try:
                    rospy.wait_for_service("/yolov5_lb_setb/start", timeout=5.0)
                    start_srv = rospy.ServiceProxy("/yolov5_lb_setb/start", Trigger)
                    result = start_srv()
                    rospy.logwarn(
                        f"[watchdog] YOLO restart result: success={result.success}"
                    )
                    # NOTE: do NOT reset _last_object_msg_time here
                    # Only real incoming messages from YOLO should reset it
                    # This way if the restart didn't actually fix it, we'll know
                    last_restart = time.time()
                except Exception as e:
                    rospy.logerr(f"[watchdog] YOLO restart failed: {e}")
                    last_restart = time.time()  # back off even on failure to avoid spam
