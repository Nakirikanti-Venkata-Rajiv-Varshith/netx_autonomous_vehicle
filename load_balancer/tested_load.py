...
class LoadBalancerNode:
    def __init__(self, name):
        ...
        self._resource_cache = {"cpu_percent": 0.0, "gpu_percent": 0.0, "ram_percent": 0.0, "power_mw": 0}
        ...
        self.bandwidth_high_threshold = rospy.get_param('~bandwidth_high_threshold',  11)  # Mbps
        self.bandwidth_low_threshold  = rospy.get_param('~bandwidth_low_threshold',   11)  # Mbps
        ...
        while self.is_running:
            try:
                ...
                for line in proc.stdout:
                    ...
                    pwr_m = re.search(r"POM_5V_IN (\d+)/(\d+)", line)
                    if pwr_m:
                        cache["power_mw"] = int(pwr_m.group(1))
